...
e="tdc-header-custom-class");a.find(".tdc-content-wrap").prepend('<div id="td_uid_start_zone_content_wrap" class="tdc-zone '+e+'" '+b+'><div class="tdc_zone"><div class="tdc-row"><div class="vc_row wpb_row td-pb-row"><div class="tdc-column"><div class="td-pb-span12 wpb_column vc_column_container"><div class="wpb_wrapper"></div></div></div></div></div></div></div>')}},init:function(){window.addWrappers=function(a,b,c){a=a.find(b);a.find(".tdc-zone").length||a.find(".tdc-row").wrapAll('<div class="tdc-zone" data-type="tdc_content"></div>');
...
b.append(a)})};window.addLoopWrappers=function(a){a.find(".td-main-content").wrap('<div class="tdc-loop-content"></div>')};window.addSidebarWrappers=function(a){a.find(".td-main-sidebar").wrap('<div class="tdc-sidebar-content"></div>')};window.checkTdBlockWrap=function(a){a.each(function(a,b){a=k(b);0===parseInt(a.height())?a.addClass("tdc-block-empty"):a.removeClass("tdc-block-empty")})};tdcAdminIFrameUI.checkIframe(tdcAdminIFrameUI._$liveIframe)},loadIframe:function(){(tdDetect.isIe8||tdDetect.isIe9||
...
"selected";break;case "landscape":l="selected";break;case "portrait":n="selected";break;case "phone":g="selected"}d+='<div class="tdc-tab-responsive"><div class="tdc-responsive-buttons">';tdcState.isMobileComposer()?d=d+('<div class="tdc-responsive-all tdc-responsive-tablet-landscape '+e+'" title="Landscape viewport"><span class="tdc-tablet-landscape-icon"></span></div><div class="tdc-responsive-phone ')+(g+'" title="Portrait viewport"><span class="tdc-phone-icon"></span></div>'):(d+='<div class="tdc-responsive-all '+
...
a.get("tag")){var b=jQuery("body .tdc-property-woo_search_version select").val(),c=jQuery("body .tdc-woos-dropdown"),d=jQuery("body .tdc-woos-regular");""===b?d.hide():"regular"===b&&c.hide()}})()}},_bindGroupAndGetHtml:function(a,c,b){var e="",d=b.get("attrs");if("tdc_zone"===b.get("tag"))var f=d.type;for(var h=0;h<c.params.length;h++){var l=c.params[h].param_name,n=c.params[h].group;if(a===tdcSidebarPanel._defaultGroupName&&_.isEmpty(c.params[h].group)||c.params[h].group===a){if(!_.isUndefined(f)&&
(!_.isUndefined(c.params[h].zone_group)&&f!==c.params[h].zone_group||"type"===l))continue;e+=tdcSidebarPanel._bindParamAndGetHtml(c.params[h],b)}if("block_template_id"===l){n=d.block_template_id;_.isEmpty(n)&&(n=window.tdcAdminSettings.globalBlockTemplate);l=window.tdcAdminSettings.mappedBlockTemplates;for(var g=0;g<l[n].params.length;g++)if(a===tdcSidebarPanel._defaultGroupName&&_.isEmpty(l[n].params[g].group)||l[n].params[g].group===a)e+=tdcSidebarPanel._bindParamAndGetHtml(l[n].params[g],b)}else if(0===
l.indexOf("tds_")){var m=l.split("-");g=m[0];var k;2===m.length?k=m[1]:2<m.length&&new tdcNotice.notice('Param "'+l+'" must have up to at most one "-" separator ',!0,!1);l=d[l];_.isEmpty(l)&&(l=window.tdcAdminSettings.globalStyle[g]);m=window.tdcAdminSettings.settingsStyle[g];for(g=0;g<m[l].params.length;g++)if(n==a||_.isUndefined(n)&&a===tdcSidebarPanel._defaultGroupName){var q=JSON.parse(JSON.stringify(m[l].params[g]));q.param_name=l+"-"+q.param_name;_.isUndefined(k)||(q.param_name+="-"+k);e+=tdcSidebarPanel._bindParamAndGetHtml(q,
...
clearPanel:function(){tdcSidebarPanel._deletePanel()},blurInputPanel:function(){jQuery("body").find("input").blur()},_addPresetValuesSelector:function(a,c){c='<div class="tdc-preset-values tdc-sidebar-horizontal-separator"><i class="tdc-font-fa tdc-font-fa-cube"></i>Pre-made style presets<i class="tdc-font-fa tdc-font-fa-angle-right"></i></div><ul class="tdc-preset-values-list" data-model_id="'+(c.cid+'">');for(var b=1;b<a.length;b++){var e="";_.isUndefined(a[b].tdc_preset_individual)||(e='data-individual="1"');
var d="";_.isUndefined(a[b].tdc_preset_class)||(d=' class="'+a[b].tdc_preset_class+'"');c+="<li "+e+" "+d+' data-values="'+window.btoa(JSON.stringify(a[b]))+'">'+a[b].tdc_preset_name+"</li>"}c+="</ul>";tdcSidebarPanel._hook.hasCallback("panel_rendered",tdcSidebarPanel.callbackPresets)||tdcSidebarPanel._hook.addAction("panel_rendered",tdcSidebarPanel.callbackPresets);return c},callbackPresets:function(){jQuery(".tdc-preset-values").click(function(){jQuery(this).next(".tdc-preset-values-list").toggle()});
